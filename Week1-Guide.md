...
- [ ] Unity 프로젝트 생성 (RooftopRunner)
- [ ] 폴더 구조 설정 (Scripts, Prefabs, Materials 등)
- [ ] Ground Plane 생성 및 Material 적용
- [ ] 빌딩 3~5개 배치
- [ ] Player Capsule 생성 및 CharacterController 추가
- [ ] PlayerController 스크립트 작성 및 적용
- [ ] WASD 이동 작동 확인
- [ ] 점프 및 더블 점프 작동 확인
- [ ] CameraFollow 스크립트 작성 및 적용
- [ ] 3인칭 카메라 작동 확인
- [ ] 마우스로 카메라 회전 확인
...
#### 7-2. 확인 사항
- [ ] 게임 시작 시 패키지 3개가 하늘에서 떨어지는가?
- [ ] 패키지가 땅에 떨어져서 굴러가는가?
- [ ] 플레이어가 패키지에 가까이 가면 끌려오는가? (자력 효과)
- [ ] 플레이어가 패키지와 충돌하면 사라지는가?
- [ ] Console에 "Package Collected!" 메시지가 뜨는가?
- [ ] 2초 후 새로운 패키지가 스폰되는가?
...
- [ ] Package 큐브 생성 및 Material 적용
- [ ] Rigidbody 추가 (중력 적용)
- [ ] Box Collider를 Trigger로 설정
- [ ] Package 스크립트 작성 및 적용
- [ ] Package를 Prefab으로 생성
- [ ] 스폰 포인트 5~8개 배치
- [ ] ObjectPool 스크립트 작성 및 설정
- [ ] PackageSpawner 스크립트 작성 및 설정
- [ ] 초기 3개 패키지 스폰 확인
- [ ] 수집 시 패키지 사라짐 확인
- [ ] 자력 효과 작동 확인
- [ ] 2초 후 재스폰 확인
...
#### 10-2. 확인 사항
- [ ] 좌측 상단에 "📦 수집: 0/15" 표시되는가?
- [ ] 타이머가 "⏰ 남은시간: 3:00"에서 감소하는가?
- [ ] 패키지 수집 시 "수집: 1/15"로 증가하는가?
- [ ] 15개 수집 시 클리어 패널이 나타나는가?
- [ ] "다시 시작" 버튼 클릭 시 씬이 재시작되는가?
- [ ] "종료" 버튼 클릭 시 게임이 종료되는가? (에디터는 플레이 모드 종료)
...
**UI**:
- [ ] 수집 개수가 실시간으로 업데이트되는가?
- [ ] 타이머가 정상적으로 감소하는가?
- [ ] 30초 이하일 때 빨간색으로 변하는가?

**게임 종료**:
- [ ] 15개 수집 시 클리어 패널이 뜨는가?
- [ ] 시간 초과 시 게임오버 패널이 뜨는가?
- [ ] "다시 시작" 버튼이 작동하는가?
...
- [ ] 트램펄린 오브젝트 생성
- [ ] Trampoline 스크립트 구현
- [ ] PlayerController에 ForceJump 메서드 추가
- [ ] 트램펄린 파티클 효과
- [ ] 트램펄린 3~5개 배치
- [ ] 빌딩 높이 다양화
- [ ] (선택) 빌딩 옥상 플랫폼 추가
